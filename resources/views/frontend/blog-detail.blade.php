            ...
            <div class="col-xl-8 col-lg-8 col-md-12">
                    ...
                    <div class="card-body">
                        @if ($details->image1)
                            <div class="item7-card-img">
                                <img src="{{ asset('assets/images/blogs') }}/{{ $details->image1 }}"
                                    alt="{{ $details->titre }}" class="w-100">
                        ...
                        <div class="item7-card-desc d-flex mb-2 mt-3">
                            ...
                            <div class="ms-auto">
                                <a href="#"><i class="fa fa-comment-o text-muted me-2"></i>{{ $nombres }}
                                    Commentaire(s)</a>
                            </div>
                        </div>
                        <a href="#" class="text-dark">
                            <h2 class="font-weight-semibold">{{ $details->titre }}</h2>
                        </a>
                        <p>{!! $details->description1 !!}</p>

                        @if ($details->image2)
                            <div class="item7-card-img">
                                <img src="{{ asset('assets/images/blogs') }}/{{ $details->image2 }}"
                                    alt="{{ $details->titre }}" class="w-100">
                                <div class="item7-card-text">
                                    <span class="badge badge-info">Blog</span>
                                </div>
                        ...
                        @if ($details->image3)
                            <div class="item7-card-img">
                                <img src="{{ asset('assets/images/blogs') }}/{{ $details->image3 }}"
                                    alt="{{ $details->titre }}" class="w-100">
                                <div class="item7-card-text">
                                    <span class="badge badge-info">Blog</span>
                                </div>
                        ...
                        <div class="item">
                            <div class="card">
                                <div class="item-card7-imgs">
                                    <a href="{{ route('blog.detail', ['slug_blog' => $blog->slug_blog]) }}"></a>
                                    @if ($blog->image1)
                                        <img src="{{ asset('assets/images/blogs') }}/{{ $blog->image1 }}"
                                            alt="{{ $blog->titre }}" class="cover-image">
                                    @else
                                        <img src="{{ asset('assets/images/blogs/f1.jpg') }}" alt="{{ $blog->titre }}"
                                            class="cover-image">
                                    @endif
                                </div>
                                <div class="card-body">
                                    <div class="item-card7-desc">
                                        <a href="{{ route('blog.detail', ['slug_blog' => $blog->slug_blog]) }}"
                                            class="text-dark">
                                            <h4 class="font-weight-semibold">{{ Str::limit($blog->titre, 20) }}</h4>
                                        </a>
                                    </div>
                                </div>
                            </div>
                        ...
                        @foreach ($commentaires as $commentaire)
                            <div class="media mt-0 p-5">
                                ...
                                <div class="media-body">
                                    <h5 class="mt-0 mb-1 font-weight-semibold">{{ $commentaire->nom }}
                                        <span class="fs-14 ms-0" data-bs-toggle="tooltip" data-bs-placement="top"
                                            title="" data-bs-original-title="verified"><i
                                                class="fa fa-check-circle-o text-success"></i> @php echo \Carbon\Carbon::parse($commentaire->created_at)->diffForHumans(null, false, 'fr'); @endphp </span>
                                    </h5>
                                    ...
                                    <a href="{{ route('blog.comment.rep', ['slug_blog' => $details->slug_blog, 'id' => $commentaire->identifiant]) }}"
                ...
                <div class="card mb-lg-0">
                    ...
                    <form class="card-body" method="POST"
                        action="{{ route('blog.comment', ['slug_blog' => $details->slug_blog]) }}">
                        @csrf
                                ...
                                <div class="video-list">
                                    <div class="video-list-inner video" style="cursor: pointer">
                                        <div class="play card-img">
                                            @if ($details->video2)
                                                <img src="{{ asset('assets/videos/posters') }}/{{ $details->video2 }}"
                                                    alt="{{ $details->video2 }}" class="play">
                                            @else
                                                <img src="{{ asset('assets/videos/posters/f1.jpg') }}"
                                                    alt="{{ $details->video2 }}" class="play">
                                            @endif
                                        </div>
                                        ...
                                        <video class="hide" muted
                                            src="{{ asset('assets/videos') }}/{{ $details->video1 }}" controls
                                            poster="{{ asset('assets/videos/posters') }}/{{ $details->video2 }}">
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card">
                                <div class="item-card7-imgs">
                                    ...
                                    @if ($blog->image1)
                                        <img src="{{ asset('assets/images/blogs') }}/{{ $blog->image1 }}"
                                            alt="{{ $blog->titre }}" class="cover-image">
                                    @else
                                        <img src="{{ asset('assets/images/blogs/f1.jpg') }}"
                                            alt="{{ $blog->titre }}" class="cover-image">
                                    @endif
                                ...
                                <div class="card-body">
                                    <div class="item-card7-desc">
                                        <a href="{{ route('blog.detail', ['slug_blog' => $blog->slug_blog]) }}"
                                            class="text-dark">
                                            <h4 class="font-weight-semibold">{{ $blog->titre }}</h4>
                                        </a>
                                    </div>
                                </div>
                            </div>
    ...
    <div class="modal fade" id="Comment" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <form class="modal-content" method="POST"
                action="{{ route('blog.comment.rep', ['slug_blog' => $details->slug_blog, 'id' => $commentaire->identifiant]) }}">
                @csrf
                @if (Session::has('success'))
                    <div class="alert alert-success" role="alert">{{ Session::get('success') }}</div>
                @endif
